 ...
 */
typedef struct {
    KeySym* symbols;        // array of KeySym indexed by key code with min_code corresponding to index 0
 ...
 *
 * NB: not thread safe and is supposed to be called only when holding AWT lock.
 */
extern void
 ...
 * which greatly reduces delays when working with the remote X server.
 *
 * NB: not thread safe and is supposed to be called only when holding AWT lock.
 */
extern KeySym
