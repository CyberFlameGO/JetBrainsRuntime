...
public final class XToolkit extends UNIXToolkit implements Runnable {
    ...
    public static final class Tracer {
        ...
        static {
            GetPropertyAction gpa = new GetPropertyAction("sun.awt.x11.trace");
        ...
        private static class AwtLockTracer implements SunToolkit.AwtLockListener {
            private long lastLockedTimeMs;               // when the locking has occurred
            private StackTraceElement lastLockedFrame;   // the frame that called awtLock()
                ...
                if (awtLockerFrame != null) {
                    ...
                    if (lockCount == 1) {
                        if (isInterestedInMethod(mname)) {
                            lastLockedFrame = awtLockerFrame;
                            lastLockedTimeMs = System.currentTimeMillis();
                        }
                    }
                }
            }
            ...
            public void beforeAwtUnlocked() {
                lockCount--;
                if (lockCount < 0) {
                    traceError("Unbalanced awtUnlock()");
                }
                ...
                if (awtLockerFrame != null) {
                    ...
                    if (lockCount == 0) {
                        if (isInterestedInMethod(mname)) {
                            final long curTimeMs = System.currentTimeMillis();
                            final long timeSpentMs = curTimeMs - lastLockedTimeMs;
                            lastLockedTimeMs = curTimeMs;
        ...
        private static synchronized void updateStatistics(String fname, long timeSpentMs) {
            if ((flags & TRACESTATS) != 0) {
                ...
                final MethodStats descr = methodTimingTable.computeIfAbsent(fname, k -> new MethodStats());
        ...
        private static class TraceReporter implements Runnable {
            ...
            public void run() {
                ...
                traceRawLine("Legend: <avg time> ( <times called> x [ <fastest time> - <slowest time> ] ms) --- <frame that called XToolkit.awtUnlock()>");
            }
        }
    }
